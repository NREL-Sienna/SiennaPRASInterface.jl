...
Base.show(io::IO, fp::FlowProblem) = print(
    io,
    "FlowProblem with $(length(fp.nodes)) nodes " * "and $(length(fp.edges)) edges",
)

Base.print(io::IO, fp::FlowProblem) = print(
    io,
    "FlowProblem with $(length(fp.nodes)) nodes " * "and $(length(fp.edges)) edges",
)
