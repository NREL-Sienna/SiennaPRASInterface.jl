struct MeanEstimate
    ...
    function MeanEstimate(est::Real, stderr::Real)
        stderr >= 0 || throw(
            DomainError(stderr, "Standard error of the estimate should be non-negative"),
        )
...
Base.isapprox(x::MeanEstimate, y::MeanEstimate) =
    isapprox(x.estimate, y.estimate) && isapprox(x.standarderror, y.standarderror)
...
function stringprecision(x::MeanEstimate)
    if iszero(x.standarderror)
        ...
        reduced = round(Int, rounded / 10.0^digits)
...
struct LOLE{N, L, T <: Period} <: ReliabilityMetric
    ...
    function LOLE{N, L, T}(lole::MeanEstimate) where {N, L, T <: Period}
        val(lole) >= 0 ||
            throw(DomainError(val, "$val is not a valid expected count of event-periods"))
        new{N, L, T}(lole)
    end
end
...
struct EUE{N, L, T <: Period, E <: EnergyUnit} <: ReliabilityMetric
    ...
    function EUE{N, L, T, E}(eue::MeanEstimate) where {N, L, T <: Period, E <: EnergyUnit}
        val(eue) >= 0 ||
            throw(DomainError("$val is not a valid unserved energy expectation"))
        new{N, L, T, E}(eue)
    end
end
