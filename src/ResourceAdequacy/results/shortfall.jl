...
getindex(x::AbstractShortfallResult, ::Colon, t::ZonedDateTime) = getindex.(x, x.regions, t)
...
LOLE(x::AbstractShortfallResult, ::Colon, t::ZonedDateTime) = LOLE.(x, x.regions, t)

LOLE(x::AbstractShortfallResult, r::AbstractString, ::Colon) = LOLE.(x, r, x.timestamps)

LOLE(x::AbstractShortfallResult, ::Colon, ::Colon) =
    LOLE.(x, x.regions, permutedims(x.timestamps))

EUE(x::AbstractShortfallResult, ::Colon, t::ZonedDateTime) = EUE.(x, x.regions, t)

EUE(x::AbstractShortfallResult, r::AbstractString, ::Colon) = EUE.(x, r, x.timestamps)
...
struct ShortfallResult{N, L, T <: Period, E <: EnergyUnit} <:
    ...
    function ShortfallResult{N, L, T, E}(
        ...
        new{N, L, T, E}(
            ...
            eventperiod_mean,
            eventperiod_std,
            eventperiod_region_mean,
            eventperiod_region_std,
            eventperiod_period_mean,
            eventperiod_period_std,
            eventperiod_regionperiod_mean,
            eventperiod_regionperiod_std,
            shortfall_mean,
            shortfall_std,
            shortfall_region_std,
...
LOLE(x::ShortfallResult{N, L, T}) where {N, L, T} =
    LOLE{N, L, T}(MeanEstimate(x.eventperiod_mean, x.eventperiod_std, x.nsamples))

function LOLE(x::ShortfallResult{N, L, T}, r::AbstractString) where {N, L, T}
    i_r = findfirstunique(x.regions, r)
    return LOLE{N, L, T}(
        MeanEstimate(
            x.eventperiod_region_mean[i_r],
...
function LOLE(
    ...
    return LOLE{1, L, T}(
        MeanEstimate(
            x.eventperiod_regionperiod_mean[i_r, i_t],
...
function getindex(x::ShortfallSamplesResult{N, L, T, P, E}) where {N, L, T, P, E}
...
function LOLE(
    x::ShortfallSamplesResult{N, L, T},
    r::AbstractString,
    t::ZonedDateTime,
) where {N, L, T}
    i_r = findfirstunique(x.regions, r)
...
EUE(
    ...
    r::AbstractString,
    t::ZonedDateTime,
) where {N, L, T, P, E} = EUE{1, L, T, E}(MeanEstimate(x[r, t]))
