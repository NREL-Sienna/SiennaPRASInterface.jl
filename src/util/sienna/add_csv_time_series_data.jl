...
function add_csv_time_series!(
    ...
    for key in keys(cc_restrictions_util_file)
        ...
        length(max_cap_idx_DA) > 1 ?
        push!(
            cc_restrictions_dict_DA,
            PSY.get_name(cc_gens_DA[max_cap_idx_DA[1]]) => cc_gens_DA,
        ) :
        push!(
            cc_restrictions_dict_DA,
            PSY.get_name(cc_gens_DA[max_cap_idx_DA]) => cc_gens_DA,
        )
        length(max_cap_idx_RT) > 1 ?
        push!(
            cc_restrictions_dict_RT,
            PSY.get_name(cc_gens_RT[max_cap_idx_RT[1]]) => cc_gens_RT,
        ) :
        push!(
            cc_restrictions_dict_RT,
            PSY.get_name(cc_gens_RT[max_cap_idx_RT]) => cc_gens_RT,
        )
    end
    ...
    if (~("Generator.csv" in csv_locations))
        error(
            "Generator Availability time series data not available for scenario $(add_scenario).",
        )
    end
    ...
    if (
        ~(1 <= days_of_interest.start <= len_ts_data_DA) ||
        ~(1 <= days_of_interest.stop <= len_ts_data_DA)
    )
    ...
    start_datetime_DA =
        start_datetime_DA + Dates.Hour((period_of_interest.start - 1) * sys_DA_res_in_hour)
    ...
    for asset_name in names(df_outage_profile)
        ...
        if (asset_name in keys(cc_restrictions_dict_DA))
            ...
            try
                ...
                PSY.add_time_series!(
                    sys_DA,
                    PSY.get_component(PSY.Generator, sys_DA, asset_name),
        ...
        if (asset_name in keys(cc_restrictions_dict_RT))
            ...
            try
                ...
                PSY.add_time_series!(
                    sys_RT,
                    PSY.get_component(PSY.Generator, sys_RT, asset_name),
