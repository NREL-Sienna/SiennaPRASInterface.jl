...
function systemmodel_0_5(f::File)
    ...
    start_timestamp =
        ZonedDateTime(read(metadata["start_timestamp"]), dateformat"yyyy-mm-ddTHH:MM:SSz")
    ...
    has_regions || error("Region data must be provided")

    has_generators ||
        has_generatorstorages ||
    ...
    if has_generators
        ...
        gen_names, gen_categories, gen_regionnames =
            readvector.(Ref(gen_core), [:name, :category, :region])

        gen_regions = getindex.(Ref(regionlookup), gen_regionnames)
        region_order = sortperm(gen_regions)

        generators = Generators{N, L, T, P}(
            gen_names[region_order],
            gen_categories[region_order],
    ...
    if has_storages
        ...
        stor_names, stor_categories, stor_regionnames =
            readvector.(Ref(stor_core), [:name, :category, :region])

        stor_regions = getindex.(Ref(regionlookup), stor_regionnames)
        region_order = sortperm(stor_regions)

        storages = Storages{N, L, T, P, E}(
            stor_names[region_order],
            stor_categories[region_order],
    ...
    if has_generatorstorages
        ...
        genstor_names, genstor_categories, genstor_regionnames =
            readvector.(Ref(genstor_core), [:name, :category, :region])
        ...
        generatorstorages = GeneratorStorages{N, L, T, P, E}(
            genstor_names[region_order],
            genstor_categories[region_order],
    ...
    if has_interfaces
        ...
        for i in 1:n_interfaces
            ...
            elseif from_region == to_region
                error(
                    "Cannot have an interface to and from " *
        ...
        interfaces =
            Interfaces{N, P}(from_regions, to_regions, forwardcapacity, backwardcapacity)

        interface_lookup = Dict(
            (r1, r2) => i for (i, (r1, r2)) in enumerate(tuple.(from_regions, to_regions))
        )
        ...
        for i in 1:n_lines
            ...
            elseif from_region == to_region
                error(
                    "Cannot have a line ($(line_names[i])) to and from " *
                    "the same region ($(from_region))",
                )
            end
        end
        ...
        line_interfaces =
            getindex.(Ref(interface_lookup), tuple.(line_fromregions, line_toregions))
        interface_order = sortperm(line_interfaces)

        lines = Lines{N, L, T, P}(
            line_names[interface_order],
            line_categories[interface_order],
        ...
        interfaces = Interfaces{N, P}(Int[], Int[], zeros(Int, 0, N), zeros(Int, 0, N))
